...
class Usdjpy(APIView):
    ...
    def get(self, request, format=None, ):
        ...
        cursor = conn.cursor()
        sql = "SELECT dates FROM exechangerate where exechange_name='일본 엔/달러' ORDER BY dates desc LIMIT 1"
        cursor.execute(sql)
        ...
        if (str(finaldate - todaydate)[0] != '0'):
            ...
            for i in data["rates"]:
                ...
                cursor = conn.cursor()
                sql = "insert into exechangerate(dates, exechange_name, symbol, price) values('{}', '일본 엔/달러', 'USDJPY', {})".format(
                    i, data["rates"][i]['JPY'])
        ...
        sql = "SELECT dates FROM usdjpy ORDER BY dates desc LIMIT 1"
        cursor2 = conn.cursor()
        cursor2.execute(sql)
        ...
        if (str(rows[0]['dates'] - today)[0] != '0'):
            ...
            for item in items:
             # 아래 두 줄은 앞에 탭을 입력하셔야 합니다...
                ...
                sql = "insert into usdjpy(dates, price, open, high, low, keyword) values('{}', {}, {}, {}, {}, '{}')".format(
                        dates, l[1].get('data-real-value'), l[2].get('data-real-value'), l[3].get('data-real-value'),
                        l[4].get('data-real-value'), "일본/엔/달러")
        ...
        cursor4 = conn.cursor()
            ...
            # 테이블 변경
        sql3 = "select * from usdjpy order by dates asc"
        ...
        for n in result:
            # 테이블 변경
            sql4 = "update usdjpy set changedate = {} where dates = '{}'".format(n['changedate'], n['dates'])
