...
class Bitcoin(APIView):
    ...
    def get(self, request, format=None, ):
        ...
        cursor = conn.cursor()
        sql = "SELECT dates FROM bitcoin ORDER BY dates desc LIMIT 1"
        cursor.execute(sql)
        ...
        if (str(rows[0]['dates'] - today)[0] != '0'):
            ...
            # 아래 두 줄은 앞에 탭을 입력하셔야 합니다...
                ...
                sql = "insert into bitcoin(dates, price, open, high, low, keyword) values('{}', {}, {}, {}, {}, '{}')".format(
                dates, l[1].get('data-real-value').replace(",",""), l[2].get('data-real-value').replace(",",""), l[3].get('data-real-value').replace(",",""),
                l[4].get('data-real-value').replace(",",""), "비트코인/암호화폐/가상화폐")
        ...
        # 테이블 변경
        sql3 = "select * from bitcoin order by dates asc"
        ...
        for n in result:
            # 테이블 변경
            sql4 = "update bitcoin set changedate = {} where dates = '{}'".format(n['changedate'], n['dates'])
