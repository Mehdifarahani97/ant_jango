...
class wti(APIView):
    ...
    def get(self, request, format=None, ):
        ...
        cursor = conn.cursor()
        sql = "SELECT dates FROM wti ORDER BY dates desc LIMIT 1"
        cursor.execute(sql)
        ...
        if (str(rows[0]['dates'] - today)[0] != '0'):
            ...
            # 아래 두 줄은 앞에 탭을 입력하셔야 합니다...
                ...
                sql = "insert into wti(dates, price, keyword) values('{}', {}, '{}')".format(dates, l[1].get('data-real-value').replace(",",""),  "WTI/유가/텍사스")
        ...
        # 테이블 변경
        sql3 = "select * from wti order by dates asc"
        ...
        for n in result:
            # 테이블 변경
            sql4 = "update wti set changedate = {} where dates = '{}'".format(n['changedate'], n['dates'])
